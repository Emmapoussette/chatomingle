...
function mysqlexec($sql){
	...
	$db_name="chatomingle"; // Database name
...
function register(){
	...
	$sql = "INSERT 
			INTO
			  users
			   ( profilestat, fname, lname, password, email) 
			VALUES
			   (0, '$fname', '$lname', '$pass', '$email)";
...
if(mysqli_num_rows($result)>=1){
   ...
   if ($result) {
	...
	if (mysqli_query($conn,$sql)) {
	  ...
	  $sql2="UPDATE TABLE account SET profilestat=1 WHERE id=$id";
	} else {
	  echo "Error: " . $sql . "<br>" . $conn->error;
	}
}
